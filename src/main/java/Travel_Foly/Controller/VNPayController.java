...
public class VNPayController {
	...
	@Autowired
	OrderDetailHotelDAO orderDetailHotelDao;
	...
	//Hotel payment
	@GetMapping("/payhotel")
	public void hotelPayment(@RequestParam("id") Integer id, HttpServletResponse response) throws IOException{
		orderHotelDTO order = orderDetailHotelDao.OrderdetailHotelInvoice(id);
		List<Integer> invoiceId = new ArrayList<>();
		invoiceId.add(id);
		session.setAttribute("hotelId", invoiceId);
	...
	public String paymentReturn(Model model,
		...
		if(session.getAttribute("hotelId")!=null) {
			...
			List<Integer> id =session.getAttribute("hotelId");
			for(Integer index : id) {
